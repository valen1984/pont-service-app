...
console.log(
  ...
  process.env.GOOGLE_CLIENT_EMAIL ? "OK" : "MISSING"
);
console.log(
...
async function generateSchedule() {
  ...
  function getDateInBuenosAires(baseDate: Date, offsetDays: number) {
    ...
    const localStr = new Date(baseDate).toLocaleString("en-US", {
      timeZone: tz,
    });
...
app.post("/api/confirm-onsite", async (req, res) => {
  // ... tu lógica actual de cash_home intacta ...
});
...
// Procesar pago desde Wallet Brick
app.post("/api/process-payment", async (req, res) => {
  const { paramData, formData, quote } = req.body;
  const result = await processPayment(paramData, formData, quote);
  if (result.success) {
    res.json(result);
  } else {
...
app.listen(Number(PORT), "0.0.0.0", () => {
  ...
  console.log(
    `🔗 Schedule endpoint: http://localhost:${PORT}/api/schedule`
  );
});
